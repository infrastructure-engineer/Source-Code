...
$fwObject = Get-AzFirewall -Name azure-fw $firewallName
If (!$fwObject) {
...
Switch ($operation.ToLower()) {
    ...
    "start" {
        ...
        try {
            ...
            $virtualNetworkName = $fwEntity.VirtualNetwork
            $resourceGroupName = $fwEntity.resourceGroupName
        ...
        try {
            ...
            $vnet = Get-AzVirtualNetwork -ResourceGroupName $fwEntity.resourceGroupName -Name $fwEntity.VirtualNetwork
            
            #allocate first public IP
            write-verbose -message ('Allocating primary Public IP Address:' + ($fwIpConfig.PublicIPAddresses)[0])
            $primaryPublicIp = Get-AzPublicIpAddress | where-object { $_.IpAddress -eq ($fwEntity.PublicIPAddresses)[0]}
            try {
            ...
            Foreach ($pubIp in $fwEntity.PublicIPAddresses) {
